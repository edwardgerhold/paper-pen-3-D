...
The euclidean norm of our matrix, the Frobenius Norm, counting together the rows and columns, componentwise and squared, then pulling the root out of the whole sum, is the square root of $(r_x+r_y+r_z)$. The sines squared and cosines squared add up to 1 each, are a factor for the $r_n$. Each $r_n$ belongs to pair of sine and cosine.
\begin{displaymath}
\|\boldsymbol{A}\|_{Frobenius} = (r_x+r_y+r_z)^{\frac{1}{2}}
\end{displaymath}
...
&= (\sum_{i=1}^{3}r_n\cos\varphi_n^{2} + \sum_{i=1}^{3}r_n\sin\varphi_n^{2})^{\frac{1}{2}}\\
&= (\sum_{i=1}^{3}r_n(\cos\varphi_n^{2} + \sin\varphi_n^{2}))^{\frac{1}{2}}\\
&= \sqrt{r_x+r_y+r_z}\\
\end{align}
\end{displaymath}
