...
xr_x\cos(\varphi_x) + yr_y\cos(\varphi_y) + zr_z\cos(\varphi_z)\\
xr_x\sin(\varphi_x) + yr_y\sin(\varphi_y) + zr_z\sin(\varphi_z)\end{array}\rigght) = \left(\begin{array}{1}x'\\y'\end{array}\right)
\end{displaymath}
...
\newtheorem{CorollaryConstant}{Corollary. \vec{f}(\vec{0}) equals the integration constant $\vec{f}{\vec{0}}=\vec{C}$.}
...
The vector $\begin{pmatrix}12\\7\end{pmatrix}$ is an example for a translation added to each point just using a function, which returns $\begin{pmatrix}12\\7\end{pmatrix}$ on $\vec{f}({\vec{0})$ fulfilling all requirements of a continuous linear map from V to W. After examining $\vec{f}({\vec{0})$ we find out, whether our map is shifted away from the origin or not. But when we explore the constant again. Each coordinate axis contributed a part to the translation. Each Integral gave one integration constant, which in our case is a two-d vector with one constant for the x and one constant for the y. What is the best deconstruction? $\frac{1}{3}$ will not 
...
\subsection{$r_n$ as bounding constant}

When looking at sine and cosine, which add up to one, if squared and summed. And looking at the factor $r_n$, which is known as the radius $r$ or as the unit multiplier in polar coordinates, in physics one writes the letters A,B,C times sine function. When going from 3-space to 2-space, $r_n$ could enlarge the coordinates.

So i find one bound by substituting the radius into the estimation. $\|\vec{x}\| $\leq \product\frac{1}{r_n}\|\boldsymbol{A}\vec{x}\|$
...
Remark. Have to fetch the english version of the Selbstaendigkeitserklaerung over the internet.\\
...
Remark. Informal version. Same opinion.\\

I think, mathematics are "Allmende" (for the people). For everyone in the world mathematics should be freely accessible.
I missed this coordinate system in the mainline of introductory texts, lectures, and books about computer graphics and 
close to lessons about surface plots.
If you have use for, just use it. Transform your coordinates with. Or study the mathematics behind by yourself.
I think this gives better understanding of all those topics mentioned and of course the euclidean spaces.
But that´s my personal opinion, if you just want the points for the plane. The thing is two lines of code, plus
the preparation of repeating code. Repeating are angle variables, one time sine and cosine calls, axis unit length. It is
compatible with frustum and premultiplication, before projection, you can do any kind of affine transformation or three dimensional deformation on your set of points.
After transforming, you could apply more, then to the two dimensional coordinates, and maybe even save time computing,
regarding linearity rules, that you can do some stuff after transformation, like scalar multiplication, as well.
This code is free. Free for anyone, and i would appreciate, if you use it, that you don´t make a secret out of it. It 
is just a two lines of code long projection, which i couldn´t even find in Wikipedia. Nothing special. And nothing to patent.\\

Remark. Informal version. Same opinion.\\

Remark. What you can do with does not belong into the license. Hmm. Another short text to summarize the features (but better then) is an idea.
...
\begin{thebibliography}    
    ...
    \bibitem{Ferus1} \textit{Prof. Dr. em. D. Ferus, TU-Berlin},
            Lecture Script, Lineare Algebra 1+2 (german), 2000, http://pages.math.tu-berlin/~ferus/skripten.html
    \bibitem{Kuehn1} \textit{Franziska K\"uhn, Technische Universit\"at Dresden},
            Lecture Script, Lineare Algebra und analytische Geometrie 1+2, http://tu-dresden.de
