...
Version 0.3.5-unstable-parts-or-many-todo-parts-raised\\
...
\subsubsection{The composition of the functions $f \circ g$}
...
ilnction will give us a surface plot. See figure \ref{g_of_x_y_figure}
...
The neighborhood of a point (x,y,z) is a ball or a sphere with a radius r or \epsilon. The ball is complete, with inner points, the sphere just is the border of the ball. The ball got the formula $B_{r}{ (x,y,z) : x^{2} + y^{2} + z^{2} <= r }$ and the sphere collects all points around with the radius of one $B_{r}(\vec{v}) :={ (x,y,z) : x^{2} + y^{2} + z^{2} = r }$.\\
...
The factor lambda lies beetween 0 and 1. If $\lambda = 0.1$ then is $(1-\lambda) = 0.9$, if $\lambda = 0.2$ then is $(1-\lambda) = 0.8$ and so on. The result is lying on the straight line of $\vec{x} + \vec{y}$. I have taken two vectors $\vec{x}, \vec{y} \in R^2$ and drawn them. The points lie on the line beetween the two points. Be careful not to mismatch the line with another, if it´s going for example if both vectors are on one line from the origin
...
The relation $\leq$ makes it possible to compare elements.
...
I am looking topology up in my lecture scripts very often and have a raw collection, of what can be applied here.
...
I recently got into the topic and am learning more about. This section can be updated in the near future.

\subsection{Differentiability of the function version}

Remark. This is under construction.

\subsubsection{Derivatives of $\vec{f}(\vec{x}) : V \rightarrow W$ }
...
Conclusion. The first derivative represents the axis vector. The 


\subsubsection{Integration}

I think here it is time for the Lebesgue Integral.\\

But it mustnt be the only.\\

Since the function is continuous, it is possible to measure the objects, and to integrate over the surfaces, or to get some volumes together.\\
...
Remark II. Underways i broke my head a few times. The periodic function can be modeled with modulo operations, some algebraic definition of.
