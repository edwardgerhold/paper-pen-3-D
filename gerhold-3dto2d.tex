...
$e_x := (r_xcos(\phi_x), r_xsin(\phi_x) )^T$
$e_y := (r_ycos(\phi_y), r_ysin(\phi_y) )^T$
$e_z := (r_zcos(\phi_z), r_zsin(\phi_z) )^T$

Commentary One for each component of $(x,y,z)$ By multiplying with, we move the 
...
or smaller than unit length. By default the cos and sin Terms have unit 
length, if you don´t multiply with $r_x, r_y$ and $r_z$.
