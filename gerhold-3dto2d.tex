...
$\neg \left( \vec{e}_i \perp \vec{e}_j \right)$ with $i,j \in \{x,y,z\}$ where $i \neq j$. \\

The plane gives us two degrees of freedom, to go horizontal or vertical. And in a cartesian coordinate system with infinite points, we can choose any direction around a point (x,y).
...
We arrange for example with about 120 degrees\footnote{The angle between $e_x$ and $e_y$ can be a little bigger, if $e_z$ points up. I have noticed this on pictures} between each axis around the origin on a plane. The point is, the generic formula still holds.\\
...
\begin{example}
\textbf{Example}\\This is full EcmaScript 6 snippet with all neccessary informations.\\
\begin{lstlisting}
...
projecting the points afterwards with this formula. I think the alternative graphics 
algorithm up to there could find a place here.\\
