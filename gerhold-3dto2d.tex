...
The words basis, axis vectors, coordinate systems  are used interchangably. And from those choices we talk about a coordinate system.
...
\newtheorem{VectorBasisBrokenLaw}{Theorem. Breaking the rule of linear independence to map from 3-D to 2-D}
...
\newtheorem{ijkVersion}{Theorem. The fundamental theorem of transforming 3-D points into 2-D points (ijk-Notation)}
\begin{ijkVersion}
...
The euclidean norm of our matrix, the Frobenius Norm, counting together the rows and columns, componentwise and squared, then pulling the root out of the whole sum, is the square root of $(r_{x}^{2}+r_{y}^{2}+r_{z}^{2})$. The sines squared and cosines squared add up to 1 each, are a factor for the $r_{n}^{2}$. Each $r_{n}^{2}$ belongs to pair of sine and cosine. When sine and cosine squares are added, the $r_{n}^{2}$ has to be factored out first, it can not add to two.
\begin{displaymath}
...
&= (\sum_{i=1}^{3}r_{n}^{2}\cos\varphi_n^{2} + \sum_{i=1}^{3}r_{n}^{2}\sin\varphi_n^{2})^{\frac{1}{2}}\\
&= (\sum_{i=1}^{3}r_{n}^{2}(\cos\varphi_n^{2} + \sin\varphi_n^{2}))^{\frac{1}{2}}\\
...
Remark. Nice to get some excercise for this topic, which can not be solved from outside with any good excercise or motivating lecture.
...
\textbf{Example}
This is an EcmaScript 5 code example. First rotate the xy plane to point downwards. Then add the z coordinate to y. That moves the point upwards again for the z-value. This results in an orthographic projection. Which is less flexible, but still one.
\begin{example}
\begin{lstlisting}
// once
