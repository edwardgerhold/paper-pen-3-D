...
Version 0.1.10 (very drafty paper)\\
...
\texbf{Remark} On a piece of paper you see three coordinate axes pointing into three
...
What we will do in the next paragraphs\\
...
\item Derive the generic case of transforming coordinate systems down to the plane and from.
\end{enumerate}
...
points into the right directions. On the plane we use to point into three directions. This means that each component contributes a move. And the final position is the right one of the new coordinate.\\
...
if you don´t multiply the cosine and the sine with $r_x, r_y$ and $r_z$, which represent the length of the hypotenuse or the new radius of the circle around, in each basis vector. \\
...
$r = \sqrt{\vec{x} \dotproduct \vec{x}}$ = $\left(\Sigma_{i=1}^{n} \vec{x}_i^2\right)^{\frac{1}{2}}$ = $\|\vec{x}\|$\\

\texbf{\emph{Now about the basis of our vector space}}\\

The other lemma we need is the generic theorem for multiplying a vector with some orthogonal basis for some coordinate system.\\

In our case the orthogonality by 90 degrees rules of perpendicular basis vectors do not count. We arrange for example with about 120 degrees beetween each around the origin on a plane. The point is, the generic formula still holds.\\
...
\begin{example}
\texbf{Example}\\And this is some EcmaScript 6 Arrow Function returning the new point. It is not optimized for speed. Means, to repeat the sin and cosine calls.\\
