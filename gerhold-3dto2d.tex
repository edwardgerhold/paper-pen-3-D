...
\author{Edward Gerhold \\http://linux-swt.de/gerhold-3dto2d.pdf}
\city{Berlin, Germany}
\date{\today}
\maketitle
...
\subsection{Are the bounds influenced by the $r_n$-values?}

Remark. This section has no consensus.

When looking at sine and cosine, if squared and summed, they add up to one. In our case this often simplifies work with the trig functions. For example when squaring for the norm. The sin and cos terms add up to one after squaring and factoring out the multipliers in front of them.\\

Another piece of the linear map is the factor $r_n$. The length of the vector is unit length times the factor r, $1 \times r$.
$r_n$ originates in polar coordinates as radius of the circle, on which the x and y coordinates ly. The line gets longer if $r > 1$ and short if $0 < r < 1$, and disappears if $r = 0$, and changes the direction by $180^{\circ}$, if the sign is negative.\\

Our question here is, which influence has $r_n$ on the norm or the size of the vectors, and which influence has this on the results of comparing the two vectors from 3-space and 2-space.

First i try substituting the radius into the estimation, and see what happens algebraicly. $\|\boldsymbol{A}\vec{v}\| \leq $r_n$\|\vec{v}\|$\\

Secondly i have no plan, what this says.

Third

Remark. Has to be examined, what kind of influence the r value has.
...
The euclidean space, the well known 2-D and 3-D spaces, are themselves so called K-Vectorspaces.\\

A $\mathbb{K}$-Vectorspace V over a body $\mathbb{K} = \mathbb{R}$ or $\mathbb{K} = \mathbb{C}$ is a set $V$ with two operations $+, \cdot$. Written as ring it is written as $(V, +, \cdot)$.\\

The vector space possesses the addition $+: V \times V \rightarrow V, (\vec{v},\vec{w}) \mapsto \vec{v}+\vec{w}$ and the scalar multiplication $\cdot: K \times V \rightarrow V, (\lambda, \vec{v}) \mapsto \lambda\vec{v}$.\\

To become a K vector space, the vector space must fulfill the following axioms.\\


\begin{enumerate}
\item $\vec{a}+(\vec{b}+\vec{c}) = (\vec{a}+\vec{b})+\vec{c}$ (associativity)
\item $\vec{a} + \vec{a}' = 0 = \vec{a}' + \vec{a} \forall a \in V$ (additive inverse)
\item $1\vec{a} = \vec{a}$    (1 is an identity operator)
\item
\item
\item
\item
...
and Edward only wrote it up for you. But why should one? This project is interesting, but nothing to patent with.
Regarding real laws, this is copyrighted 2015 by Edward Gerhold, and permission is granted for anyone to use the 
transformation. \\

Remark. Formulation lacks.\\
...
% lecture scripts english
    \bibitem{Toplogy} \textit{ ,  },
            Lecture Script, Topology Notes, Math 240
    \bibitem{Functional1} \textit{},
            Lecture Script, Functional Analysis,    
    \bibitem{Functional2} \textit{},
            Lecture Script, 
    \bibitem{Functional3} \textit{},
            Lecture Script,
% lecture scripts in german
    ...
    \bibitem{Ferus2} \textit{Dirk Ferus, TU-Berlin, em.},   
            Lecture Script, Analysis II, 2008, http://page.math.tu-berlin/~ferus/skripten.html            
