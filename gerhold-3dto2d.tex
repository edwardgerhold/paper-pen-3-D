...
Version 0.3.6-unstable-parts-or-many-todo-parts-raised\\

\textbf{Remark. This is a development version. And has chaotic parts} This file contains typos, unwanted logical mistakes and miscounts, and a maybe accidental letters, which came from a suddenly appearing double cursor in the editor. This is my first \LaTeX ever.
Status. Moved the chaotic stuff into the appendix. It is now the chaotic appendix and the current version is not as nice as earlier and cleaner ones. The good news are, the next versions will not be about expanding the topic anymore. 
}\\
...
\caption{This is $g(t)$ from \ref{g_of_t_code} plotted with the cheap3danimate.html code within $[-2,2] \times [-2,2]$.}
\end{figure}
...
The derivatives of the angles are not taken. But it seems to be obvious, that the sin and cosine would be rotating around by 90 degrees each time the derivative of the derivative with respect to the angle is taken. The slope is the vector, because the point is moving by that vector from its current position a straight line, when multiplied.\\
...
no curves, so no tangent. There is no curvature, so no second derivative. But it is perfect. And calculus is right, because the three vectors are three straight lines.
...
Conclusion. The first derivative represents the axis vector. The gradient gives us the complete coordinate system. If we use the gradient then in another composition, we can apply the mapping again. 

\begin{center}
$\nabla\vec{f} := \left(\begin{array}{1}\vec{e}_{x}\\\vec{e}_{y}\\\vec{e}_{z}\\\end{array}\right) = \boldsymbol{A}^{T} $
\end{center}

Looks like we have to transpose the row vector again. Because the gradient seems to be the same as the 3 by 2 matrix $A^{T}$ and not like our original coordinate system.

\begin{center}
$(\nabla\vec{f})^{T} := \begin{pmatrix}\vec{e}_{x} & \vec{e}_{y} &\vec{e}_{z}\\\end{pmatrix} = \boldsymbol{A} $
\end{center}

Now we could reuse the vector of vectors (the matrix) and multiply again with coordinates. But before, we come to another conclusion, which i had underways, after writing down the transposed gradient, the next morning, reading a lecture script about Analysis 2 (vector calculus).

\begin{center}
$(\nabla\vec{f})^{T} \Leftrightarrow \boldsymbol{J}(\vec{f}(\vec{x})) := \begin{pmatrix}\partial_{1}f_{1} & \partial_{2}f_{1} & \partial_{3}f_{1}\\\partial_{1}f_{2} & \partial_{2}f_{2} & \partial_{3}f_{2}\end{pmatrix}$
\end{center}

The transposed gradient of the vector function $\vec{f}(\vec{x}) : V \rightarrow W$ is the Jacobi Matrix, which is equal to the matrix, i discussed already. This possible makes another re-ordering neccessary. But first look yourself.\\
...
The factor lambda lies beetween 0 and 1. If $\lambda = 0.1$ then is $(1-\lambda) = 0.9$, if $\lambda = 0.2$ then is $(1-\lambda) = 0.8$ and so on. The result is lying on the straight line beetween $\vec{x}$ and $\vec{y}$. I have taken two vectors $\vec{x}, \vec{y} \in R^2$ and drawn them. The points lie on the line beetween the two points. Be careful not to mismatch the line with another, if it´s going for example if both vectors are on one line from the origin
...
\subsection{Algebra and related}

The last section is also the last i come to to study it for this article. Algebra.
It is possible, that the section moves, but first i have to complete my research.


\subsubsection{K-Vectorspace and Modulns}
...
\subsection{Differentiability of the function version}

\subsubsection{Integration}

Since the function is continuous, it is possible to measure the objects, and to integrate over the surfaces, to get some areas and volumes together.\\

Remark. More TODO.

A big lack in this document is, that the estimations are not ready.\\
...
Remark II. Underways i broke my head a few times. The periodic function can be modeled with modulo operations, some algebraic definition of. Else the possible zeros would blow the try up. 

Remark III. Here i am missing more experience in Ring Theory and related. Up to the set theory.
