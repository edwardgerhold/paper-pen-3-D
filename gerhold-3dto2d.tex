...
Version 0.2.8 (the is getting reworked paper)\\
...
The index $_n$ in $r_n$, $\varphi_n$, $\vec{e}_n$ is the index for $_x$,$_y$,$_z$. For example $\varphi_n$  stands $\varphi_x, \varphi_y, \varphi_z$. $r_n$ stands for $r_x$, $r_y$ and $r_z$. $\vec{e}_n$ is for $\vec{e}_x$, $\vec{e}_y$, $\vec{e}_z$ It is possible, that in the formulas $x,y,z$ and $1,2,3$ may be used interchangeably. For example, when summing up the products of the coordinate componentents with basis components, this happens. The formula is $\sum_{i=1}^{3}\vec{x}_i\vec{e}_i$, which is a sum of $x,y,z$ and the $\cos \varphi_n$ terms in the first components of $\vec{e}_n$ for $x'$ and a sum of $x,y,z$ and the $\sin \varphi_n$ terms in the seconds components of $\vec{e}_n$ for $y'$.
...
More advanced measurements include the p-Norm, which is $\sqrt[p]{\sum_{i=1}^{n}|\vec{x}_i|^{p}} 1 \leq p \leq \infty$ and $\sup_{i=1..n} |\vec{x}_i|$ for $p=\infty}$ and the max-Norm $\|\vec{x}\|_{\infty}$= $\sup_{} \{|\vec{x}_i| \}$. There are matrix norms like $\|A\| = \max_{i=1..m} \sum_{j=1}^{n}A_{ij}$, which for example yields the largest row of a m by n matrix. Norms are used widely in mathematics for measuring the lenghts of the vectors and matrices. And the distance function $d(\vec{x},\vec{y}) = \|\vec{x}-\vec{y}\|$ is used to measure the distance between to points or two vector tips. A vector space V with a distance function $d(x,y)=\|x-y\|$. 
  is called a metric space $(V,d)$. And a complete metric space with a norm, written $(V, \|\cdot\|)$, is a Banach space. 

\subsection{$\vec{e}_n$ are the three 2-D (pseudo) basis vectors}
...
The absolute value of the dot product squared is less or equal to the product of the two norms squared. Squaring the two norms to the right brings the square root away, that you multiply with the full scalar product length.
...
\textbf{Remark} Can be that i am wrong. I had to correct myself, after i had a weak error. I think i have forgotten the double
sum when multiplying two sums. The other point is, i do not know now, whether it is right or wrong. But the right term looks larger for me now. Like the sigma is squared since $i=j$ but replacing with a square symbol is then probably wrong. (i removed it with another upload)
...
not, can not be told from looking at these matrices by a guy like me. I just see symmetric matrices and think, no, no. This topic is possibly not continued in the next versions. But of course i will clean this text up.
...
I do not now values for k now by heart, but remember the formula already. I took a skewing matrix and forgot about the empty fields.
