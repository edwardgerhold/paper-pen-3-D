...
\end{displaymath}
\begin{displaymath}
|\sum_{i=1}^{n}\vec{v}_{i}\vec{w}_{i}|^{2} \leq (\sum_{i=1}^{n}\sum_{j=1}^{n}|\vec{v}_{i}|^{2}|\vec{w}_{j}|^{2})^{\frac{1}{2}})^{2} 
=
|\sum_{i=1}^{n}\vec{v}_{i}\vec{w}_{i}|^{2} \leq \sum_{i=1}^{n}\sum_{j=1}^{n}(|\vec{v}_{i}||\vec{w}_{j}|)^{2}
\end{displaymath}

\textbf{Remark} Can be that i am wrong. I had to correct myself, after i had a weak error. I think i have forgotten the double
sum when multiplying two sums. The other point is, i do not know now, whether it is right or wrong. But the right term looks larger for me now.
