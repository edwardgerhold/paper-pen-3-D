...
\fbox{The definition of the 2x3 operator space, which now has a basis is started, but now dirty.}
...
\subsection{Defining the 2x3 as transitional space to go from 3-D to 2-D}

The $\mathbb{R}^{2\times3}$ is NOT a K-vectorspace. The axioms of a body and a vector space can not be fulfilled.\\

And the $\mathbb{R}^{2\times3}$ is NOT a regular $n \times n$ vector space.\\

A vector space defines addition and scalar multiplication.\\

\begin{displaymath}
\lambda\vec{v} \rightarrow \lambda\vec{w}
\end{displaymath}

It has addition

\begin{displaymath}
\vec{u} + \vec{v} \rightarrow \vec{w} + \vec{x}
\end{displaymath}

Both assuming a 2x3 standard basis. The result of the operations with the 3-D vectors are 2-D vectors.\\

So, ok, we have both. $Av + Az = A(v + z) = w$, and $\lambdaA(v) = A(\lambdaw)$ in the previous definitions.
Assume that A is now the basis of the 2x3. Right. That does not make a usable vector space out of the 2x3.
Can you spot one difficulty in our vector space? Right at once our 3-D input turns into 2-D vectors, and
we continue operating on R2 or will not be able to calculate more.

So it is a vector space? Yes. And No. You can not work with vectors inside this space.

They called it the space of the 2x3 matrices, or the space of the 2x3 operators, i think i can remember correctly.

\newtheorem{Proposition2x3space}{Proposition. $\mathbb{R}^{2\times3}$ is the operator space for operators which map from 3-D to 2-D.}
\begin{Proposition2x3space}
The $\mathbb{R}^{2\times3}$ is not an ordinary vector space, in which you can do your calculations. Once you put a 3-D vector inside,
a 2-D vector is the result. You can see the $\mathbb{R}^{2\times3}$ as a one-way-street from $\mathbb{R}^{3}$ to $\mathbb{R}^{2}$ 
\end{Proposition2x3space}


\newtheorem{Proposition}{Proposition. The $\mathbb{R}^{2x3}$ is the operator space for making 2-D images of 3-D coordinates.}
\begin{Proposition}
...
to translate 3-D points into 2-D points with. The $\mathbb{R}^{2x3}$ is the transitional vector space of 2-D images made of 3-D coordinates. It maps onto $\mathbb{R}^{2}$.
\end{Proposition}\\

\fbox{The 2x3 maps from 3-D to 2-D. The result is a subset of $\mathbb{R}^{2}$.}
...
\newtheorem{PropositionD}{Proposition. The $\mathbb{R}^{2\times3}$ basis (or map operator) is arranged by three 2-D axes around the circle.}
\begin{PropositionD}
...
\fbox{This is definitly a draft to replace by the 2x3 space being the operator space from 3 to 2. }
...
Can you spot, what i did? I embedded the third axis in an orthogonal 2-D system. Here the axes point into the same positive direction, this is a lefthand system.
...
Here the additional axis shows in the other direction. This is a righthand system.

Remark. Incomplete. TODO.

Remark II. The chapter deconstructing the basis will succeed through the above standard basis, as i can use these for examples of the SVD, i already calculated RRt, RtR, LLt and LtL for the righthand and lefthand systems, much easier than with the trig functions.
This is to be written in the next days.
