...
And as i am looking into geometry and state of the art, i should mention that.
...
Remark. I think, this can be also done with Gram-Schmidt. Gram-Schmidt is an iterative process, to reduce the first vector, then with the information the second and so on. \\

\subsection{Lefthand and righthand standard bases}

Without calculating them, but concluding them from the existing, i get the following standard bases containing only zeros and ones.

\begin{displaymath}
    E^{R^{2x3}}_{lefthand} = \begin{pmatrix}1&0&1\\0&1&1\end{pmatrix}
\end{displaymath}
\begin{displaymath}
    E^{R^{2x3}}_{righthand} = \begin{pmatrix}-1&1&0\\-1&-1&1\end{pmatrix}
\end{displaymath}

Can you spot a little lack in that? Right, the column vectors are not normalized. That means, that for example the column vectors with two ones have a length $\|\vec{e}_n\| = \sqrt{2}$. The length is $\sqrt{1^{2} + 1^{2}} = \sqrt{2}$. Our trigonometric basis vectors have unit length, unless you multiply them with a new factor $r_n$. So in the lefthand coordinate system the z-axis is longer. And in the righthand system the x- and the y-axis are longer.
