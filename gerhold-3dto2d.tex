...
The euclidean norm of our matrix, the Frobenius Norm, counting together the rows and columns, componentwise and squared, then pulling the root out of the whole sum, is the square root of $2(r_x+r_y+r_z)$. The sines squared and cosines squared add up to 1 each, are a multiplyer for the $r_n$. Each $r_n$ exists twice, one for cosine, one for sine.
\begin{displaymath}
\|\boldsymbol{A}\|_{Frobenius} = (2(r_x+r_y+r_z))^{\frac{1}{2}}
\end{displaymath}
...
&= (\sum_{i=1}^{3}r_n\cos\varphi_n + \sum_{i=1}^{3}r_n\sin\varphi_n)^{\frac{1}{2}}\\
&= \sqrt{2(r_x+r_y+r_z)}\\
\end{align}
\end{displaymath}
