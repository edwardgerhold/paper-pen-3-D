...
\newtheorem{PropositionOpt2}{Proposition. The $\mathbb{R}^{2\times3}$ matrix operator is optimal for transforming 3-D into 2-D.}
...
\newtheorem{PropositionOpt3}{Proposition. The linear dependent vector can not be removed from the 2x3 basis. }
...
$((A^{T}A)-\lambda{I})x=0$ and $((AA^{T})-\lambda{I})x=0$ want to be solved, and then we want the SVD.
...
Remark. Edwards Theory for the "overlayed" or covered, or what the word is, points is, that they are on one line normal to the plane. It is covered what is orthogonal to the plane and behind the first point on that line.\\
...
\begin{displaymath}
\boldsymbol{A}\boldsymbol{A}^{T} = \begin{pmatrix}\frac32&\frac12\\\frac32&\frac12\end{pmatrix}
\end{displaymath}

Solving $det(A-\lambdaI)=0$ gives us the characteristic polynom and after foiling and simplification
...
\begin{displaymath}
\vec{\xi}_{1} = \begin{pmatrix}0\\1\\-1\end{pmatrix}
\end{displaymath}
