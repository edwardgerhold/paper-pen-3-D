...
\fbox{The sections need definitly a new ordering because of shifted priorities.}\\
We come to solve for the purpose of the real R2x3 very soon. See appendix for hints.\\
...
Corollary. If the result of $\vec{f}({\vec{0}})=\vec{w}$ does not equal zero ($\vec{0}$), one can solve for the constant c, which is the translation from the origin. If the normal result of $\vec{f}({\vec{0}})$ is $\vec{f}({\vec{0}})=\vec{0}$ then $\vec{f}({\vec{0}})=\vec{C}$.
...
Remark. Notation stuff. I used the partial symbol for the differential. The physicists could write this $\int_{0}^{3}d^{2}x\vec{e}_{x}+...$. It is just dx,dy,dz and per axis vector componentwise, because $\vec{e}_{n}$ are vectors, if somebody asks.\\

\newtheorem{CorollaryLimitsIntegration}{Corollary. Integration Limits.}
\begin{CorollaryLimitsIntegration}
To integrate the coordinates, which results in the three two-d vectors, which build a final sum of the final coordinate vector, the x coordinate is the limit from [0,x] on the first integral, y is from [0,y] and z is from [0,z] on the last integral.
\end{CorollaryLimitsIntegration}

\emph{Integration again with negative coordinates}.\\

Let us try to integrate $\begin{pmatrix}-3\\-1\\-7\end{pmatrix}$. After examining wether [-3,0] or -[0,3] is the right way to integrate negative coordinates, i come to the conclusion, integrate from [0,3] and subtract the integrals.\\
...
Integration of [-3, 0] would fail, since $0\vec{e}_{x} - (-3)\vec{e}_{x}$ would result in $3\vec{e}_{x}$. And integration from 0 to -3 is not what i have read in the books, the negative number would be the lower and not the upper limit of integration, so i conclude,
that the integrals have to be subtracted when negative coordinates are used.\\

\newtheorem{CorollaryLimitsIntegration}{Corollary. Negative Coordinates and Limits of Integration. Subtract the integrals.}
\begin{CorollaryLimitsIntegration}
To integrate negative coordinates, the limits of integration become the positive coordinates, and the integrals, which are affected are subtracted from the others. Integrating from [0,-x] or from [-x,0] would results in a positive result or be impossible by definition.
\end{CorollaryLimitsIntegration}


Remark. Chapter needs to be cleaned up.
...
\subsection{Definition of \mathbb{R}^{2\times3}}

Remark. August, 5. This document exists almost a month. It broke once in after a week, touching the broken rule of linear independence and the algebraic construction of R2x3. The former is solved, it is written, that it is good, to break this rule for this special case. The latter is still a topic, for which i have to dig in Algebra, Topology, Analysis, to find the right words. But i am getting closer and closer.\\
 
Remark. The following will be re-interpreted over the next days in August, as i think, i am gonna solve this.\\



I think rationally, this space is giving the two dimensional image of three dimensional input coordinates. Or in other words, this space takes three dimensional input coordinates. Or other words, it creates 2-D images from 3-D vectors.\\

\newtheorem{Definition}{Definition. My human definition of \mathbb{R}^{2\times3}}
\begin{Definition}
The $\mathbb{R}^{2\times3}$ space is rationally viewed, the 2-D space of three input coordinates. The images over $\mathbb{R}^{2\times3}$ are two dimensional. But the input coordinates for the image are three dimensional. Instead of getting abstract and universal, the $\mathbb{R}^{2\times3}$ is rationally the space of the 2-D image of 3-D data.
\end{Definition}\\

This means we have three ways instead of two, to move the point over the image to the right spot.\\

\newtheorem{Corollary}{Corollary. Each of the three input coordinates contributes to the two final coordinates.}
\begin{Corollary}
The $\mathbb{R}^{2\times3}$ has three axes instead of two. Since the output dimensions are less than the input dimensions, all three coordinates contribute their parts to the two coordinates.
\end{Corollary}

Maybe the following definition is a theorem. The 2x3 basis is not orthogonal.

\newtheorem{Definition}{The three axes of $\mathbb{R}^{2\times3}$ are not orthogonal.}
\begin{Definition}
The coordinate system of the $\mathbb{R}^{2\times3}$ is not a regular orthogonal coordinate system. The best way to lay the three axes out on two dimensions is around the circle, which is available in two dimensions. 
\end{Definition}


In a $\mathbb{R}^{3\times{n}}$ with $n>3$ we would use the sphere. If you think about 3x4 and additional translations, you get the point as another vector pointing from the origin of the sphere into the translations direction.\\

\newtheorem{Corollary}{The three axes of $\mathbb{R}^{2\times3}$ are linearly dependent.}
\begin{Corollary}
The coordinate system of the $\mathbb{R}^{2\times3}$ is not a regular orthogonal coordinate system. A minimum of one axis has to be dependent of the other. Regarding, that we print onto the real $R^{2}$ standard basis, all three axes are linear combinations for use with the two dimensional standard basis.
\end{Corollary}

\newtheorem{Definition}{The three axes of $\mathbb{R}^{2\times3}$ are best laid out around the circle.}
\begin{Definition}
The coordinate system of the $\mathbb{R}^{2\times3}$ is not a regular orthogonal coordinate system. The circle is available in two dimensions. The circle gives optimal freedom of arrangement. Together with the parametrization r cos angle, r sin angle for three instead of two input coordinates, each coordinate moves proportionally from the origin or the actual position on to the right destination.
\end{Definition}

The 2x3 image is describable in polar coordinates.

\newtheorem{Corollary}{The 2x3 image is optimally describable in polar coordinates.}
\begin{Corollary}
The coordinate system of the $\mathbb{R}^{2\times3}$ which is layed out around the circle is best viewed with polar coordinates. 
It is the 2-D space for a 3-D image in polar coordinates.
\end{Corollary}

Proposition: The R2x3 IS the 2-D space for 3-D in polar coordinates, like i started this chapter. It is the 2-D space of a 3-D image.\\

Remark. This is incomplete and needs review by myself between shopping and returning.\\

\fbox{this section needs review and to be overworked by me, the definition of r2x3 is important}
...
is describable. Like i did the last lines, for example. But i can not bring it. I have a leak, a lag, a lack in the topic, which is making me unsure. I have to read another teacher for, to get the final point, to understand 2/3 a 3/2 and explain 2x3 as 2x3. It is not 
...
the 2x3.
Remark. The dimension formula is 
\begin{displaymath}
dim(U)+dim(V) = dim(U\cap{V}) + dim(U+V)
\end{displaymath}
Remark. This has nothing to do with this text. Reminder. This is a dev version with chaotic parts.
...
\fbox{Remark. August 5. The 2x3 example \cite{Strang1} seems to be for and about R6. This is different.}
...
\fbox{Remark. August 5. The 2x3 basis is different from the R6 basis by purpose.}\\

Remark. August. I will work on this chapter the next days. Proving it is a 2-D image of 3 input coordinates.\\
