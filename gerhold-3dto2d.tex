...
\textbf{Remark} This subsection is not complete. In our case is probably $\|\boldsymbol{A}\vec{x}\| \leq \|\vec{x}\|$ but i am not finished with.\\


\subsection{Metrics}

Where a norm is, there is an induced metric.
The measurement of the distance between two points is defined by the d-function. It´s the length of the difference vector between the two points.\\

\begin{displaymath}
    d(\vec{x}, \vec{y})_2 = \|\vec{x}-\vec{y}\|_2
\end{displaymath}
