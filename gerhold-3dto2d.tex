...
Remark. This brought me to the conclusion to prove more rules of vector spaces. In the next versions.\\

Remark. Did i mention that this is the real $\mathbb{R}^3$ embedded in the real $\mathbb{R}^2$? I just noticed in the bus,
going over \cite{Strang1} again. \cite{Strang1} brings me to the idea of proving nullspace, row space, column space, left null space,
linearity of the operations and a question. What´s the norm $\|\vec{x}\|$ in $\mathbb{R}^{2x3}$? and is this embedding really the $\mathbb{R}^{2x3}$? These question go beyond the simple transformation of the points and concern mathematics and linear algebra. I wrote down the transpose of the matrix, and multiplied it for a 2x2 matrix with $\boldsymbol{A}\boldsymbol{A}^T$. I didn´t for the 3x3 with $\boldsymbol{A}^T\boldsymbol{A}$ in the bus. I wrote down easier numbers for the vectors for manual calculations than the sines and cosines. 

\textbf{Example}
The example from the beginning, where i define sample angles in radians looks in numbers like. Let´s round it up to full numbers to calculate manually a few examples with.
\begin{example}
\begin{displaymath}
\boldsymbol{A} = \left(    
    \begin{array}{1111}
...
Remark. This chapter has been added on June 30, 2015 and is a new draft in the draft for today.\\
