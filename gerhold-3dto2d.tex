...
\textbf{Remark. This is a development version. And has chaotic parts. And (only temporarily) some logical mathematical mistakes, which are of course going to be corrected.} This file contains typos, unwanted logical mistakes and miscounts, and a maybe accidental letters, which came from a suddenly appearing double cursor in the editor. This is my first \LaTeX but not my last. If you ever print this, print 4 on 1. This saves paper. This document is unfinished and not completly shaped.\\}

\fbox{To get the formalities right, a german translation version is now being worked out by me.}
...
Edward Gerhold, who is writing this document, is nativly speaking german. He was good in english at school, but has to admit,
that it became weak over the time. Today it is somewhat weaker than it normally was. Here and there are a few mistakes, wrong pronounciations, or simply ununderstandable phrases included under the other phrases in the document. But by accident. The author will do his best over the next time, to finish the explainings and to correct the wrong english by using a vocabulary book.\\
...
Remark. [Import Definition Basis here. It is not too much math for the beginning.]\\
...
It is easy to intermingle this with the word basis and to multiply with a basis. The calculation is the same. But it is called differently. It is called a linear combination of our axis vectors with the coordinates. And the result is a linear map. And the matrix we use induces a linear map by its $m\times n$ dimension. More about (the full story) a little later.\\

Remark [Import Definition Linear Combination here. It is not too much math for the beginning.]\\

Mathematically speaking, it is quite dramatic to intermingle these, and during writing the document, i had mostly the problem of intermingling "linear combination of coordinates and the axis vectors" with "multiplying coordinates with the basis".\\
...
\subsection{Defining the $(2\times3)\cdot(1\times3)$ as transitional space to go from 3-D to 2-D}
...
\newtheorem{PropositionOpt}{Proposition. The $\mathbb{R}^{2\times3}$ operators have infinite combination possibilities $(m\times c)\cdot(c\times n)=(m\times n)$}
\begin{PropositionOpt}
TODO
\end{PropositionOpt}
...
\newtheorem{PropositionOpt}{Proposition. The $\mathbb{R}^{2\times3}$ matrix operator is optimal for transforming 3-D into 2-D.}
\begin{PropositionOpt}
With three coordinate axes as column vectors, arranged around the circle, derived from the 2x3 standard basis, the 2x3 matrix is
an optimal operator for transforming 3x1 vectors (3-D vectors) into 2x1 vectors (2-D vectors).
\end{PropositionOpt}
...
\newtheorem{PropositionOpt}{Proposition. The linear dependent vector can not be removed from the 2x3 basis. }
\begin{PropositionOpt}
...
We test the "embedded" coordinate system with the $\mathbb{R}^{2}$ standard basis and the one linear dependent vector. Here we suspect one pair of vectors to be orthogonal, of course. It is just for completeness and overview for us, to show that. But by 
showing that, we will get another stoking result, of the cosine and sine terms being returned from the dot products.\\

$\vec{e}_{x}\cdot\vec{e}_{y} =$
$\vec{e}_{x}\cdot\vec{e}_{z} =$
$\vec{e}_{y}\cdot\vec{e}_{z} =$
