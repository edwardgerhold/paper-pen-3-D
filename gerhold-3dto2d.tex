...
\textbf{\emph{The integral needs to be fixed:}}\\

We can fix the integral, which can not be changed manually each time, by looking whether the coordinates are positive or negative.
Using the absolute value in the limits of integration and the sign function in front of the integrals make the thing work automatically for any coordinate, and we could integrate over each coordinate.\\
...
Remark. A version earlier i brought more arguments. But proving the R2x3 to be an unique space fails. It is a subspace of R2, and an Algebra taking three coordinates to combine them with coefficients into two. Some kind of R[x] is a 2x3. I have got to read Algebra.\\
...
\newtheorem{Proposition}{Proposition. The $\mathbb{R}^{2\times3}$ is the 2-D space of 3-D images}
\begin{Proposition}
The R2x3 combines three coordinates into two. For the human nature the most valuable and logical use for the 2x3 is to present the 2-D images of 3-D coordinates by feeding the R2x3 algebra with three coordinates and taking the 2-D image from.
\end{Proposition}


\newtheorem{Proposition}{Proposition. Polar coordinates are optimal for laying out three axes for $\mathbb{R}^{2\times3}$ }
\begin{Proposition}
It is the virtual 3-D coordinate system which is brought by R2x3 into the R2 is best layed out with polar coordinates, because using the trigonometric functions makes optimal arrangement possible by choosing angles for each axis. 
\end{Proposition}

The $\mathbb{R}^{2\times3}$ is NOT a K-vectorspace.\\

Several axioms do not hold. If you put three coordinates into, they will be combined into 2 coordinates. The identity operator for example can not return the same number.\\

What kind of vector space on its own it is, i can not say today, if it is. I have to continue learning higher algebra.\\

But the $\mathbb{R}^{2\times3}$ is an Algebra on $\mathbb{R}^{2}$.\\

It combines three numbers into two.\\


\newtheorem{Proposition}{Proposition. The $\mathbb{R}^{2\times3}$ Algebra maps 3-D coordinates optimally onto 2-D coordinates}
\begin{Proposition}
Functionals and matrices from R2x3 
\end{Proposition}

R2x3 is a place for operators, mediators. They provide a path from R3 to R2. It is a one-way-street.

Remark. The formulation is more stumbled than in the last version, i simply deleted all again, because the R2x3 fails the vector space test and survives the home of operators and being an algebra on R2 but i have to analyse exactly the latter again. Formerly i had no experience, but writing a few group operations into a table.


\subsection{Orthogonality in the 2x3 matrix}

The mapping from the three dimensional input to the two dimensional output can not be orthogonal for all three two dimensional axes. \\

The $\mathbb{R}^{2\times3}$ Algebra maps the three coordinates onto three vectors on the plane.\\

The R2-x coordinate combines three coefficients multiplied with the three coordinates to a sum of a horizontal or x-axis displacement.
And the R2-y coordinate combines three coefficients multiplied with the three coordinates to a sum of a vertical or y-axis displacement.\\

The mapping is onto an orthogonal R2 system.\\

Remark. The covered points (the points virtually behind a point in from) lie all on one line each. There is an infinite number of lines from the front of the viewer orthogonally out of the plane which cover points behind the nearest point to the front.

\subsection{2x3 standard basis}


Remark. This example seems to be about R6.\\
