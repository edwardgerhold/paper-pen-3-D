...
\emph{About integrating a coordinate}\\

If i take the coordinates as limits of integration from 0 to x, 0 to y, 0 to z, i probably get the transformation again.
Let us try to integrate $\begin{pmatrix}3\\1\\7\end{pmatrix}$. I have had the idea in the bus, to check this way out, too.
...
&= (3\vec{e}_{x}-0\vec{e}_{x}+\vec{C}_{1}) + (1\vec{e}_{y}-0\vec{e}_{y}+\vec{C}_{2}) + (7\vec{e}_{z}-0\vec{e}_{z}+\vec{C}_{3})\\
&= 3\vec{e}_{x} + 1\vec{e}_{y} + 7\vec{e}_{z} + \vec{C}\\
\end{align}
\end{displaymath}

Which is then summed up as a two dimensional vector, by multiplying each basis vector with the coordinate, and building a final sum.
Seems to be fine for us and equal.\\
...
% lecture scripts english
    \bibitem{Toplogy} \textit{ ,  },
            Lecture Script, Topology Notes (english), Math 240
