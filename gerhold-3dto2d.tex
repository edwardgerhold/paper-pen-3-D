...
\textbf{Remark. This is a development version. And has chaotic parts} This file contains typos, unwanted logical mistakes and miscounts, and a maybe accidental letters, which came from a suddenly appearing double cursor in the editor. This is my first \LaTeX().\\ \emph{There must be a new emphasizement made on the normed vector space and some other points throughout the text. I will change it carefully over the next days.
...
\newtheorem{VectorBasisVersion}{My fundamental theorem of transforming 3-D points into 2-D points (Vectorbasis)}
...
Remark. A Hamel-Basis requires a linearly independent set of basis vectors. Which we can not provide. We change the dimension.
...
\newtheorem{VectorBasisBrokenLaw}{Breaking the rule of linear independence to map from 3-D to 2-D}
...
Conclusion. The first derivative represents the axis vector. The gradient gives us the complete coordinate system. If we use the gradient then in another composition (with matrix vector multiplication with a three coordinate vector), we can apply the mapping again. \\
...
For the proof it is maybe neccessary, to put the $\boldsymbol{A}$ back into the parens. But ractically i would like to know the rule and then take the smallest calculation.\\

\begin{center}
$(\boldsymbol{A}v_{n}) = \boldsymbol{A}(v_{n})$\\
...
\begin{center}
\boldsymbol{A}\lim_{n\rightarrow\infty} (v_{n}) = \boldsymbol{A}\vec{w} = \vec{w}
\end{center}
...
We start at $n=1$ and when  there is some $n_{0}$ than the distance must shrink under epsilon and finally go to zero.
